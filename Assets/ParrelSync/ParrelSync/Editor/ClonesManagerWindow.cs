            ...
            else// If it is an original project...
            {
                if (isCloneCreated)
                {
                    ...
                    for (int i = 0; i < cloneProjectsPath.Count; i++)
                    {
                        ...
                        bool isRunning = ClonesManager.IsCloneProjectRunning(cloneProjectPath);

                        // Display clone status
                        EditorGUILayout.LabelField($"Clone {i} {(isRunning ? "(Running)" : "")}", isRunning ? EditorStyles.boldLabel : EditorStyles.label);

                        // Display clone project path
                        GUILayout.BeginHorizontal();
